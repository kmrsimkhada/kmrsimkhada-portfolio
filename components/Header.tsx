...
const Header: React.FC = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'h-20 bg-background/80 backdrop-blur-md shadow-lg shadow-surface/20' : 'h-24'}`}>
      <div className="max-w-7xl mx-auto px-6 sm:px-8 h-full flex justify-between items-center">
        ...
        <div className="md:hidden">
          <button onClick={toggleMenu} className="z-50 relative w-8 h-8 text-accent focus:outline-none" aria-label="Open menu">
      ...
      {/* Mobile Menu */}
      <aside className={`fixed top-0 right-0 h-full w-full max-w-xs bg-surface transform transition-transform duration-300 ease-in-out z-40 ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            ...
            {navLinks.map((link) =>
              link.isRoute ? (
                <Link
                  ...
                  to={link.href}
                  onClick={closeMenu}
                  className="text-lg text-text-primary hover:text-accent transition-colors duration-300"
                >
                  {link.label}
                </Link>
              ) : (
