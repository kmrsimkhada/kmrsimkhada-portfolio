...
const Hero: React.FC<HeroProps> = ({ name }) => {
  ...
  const two = <h1 className="text-4xl sm:text-6xl md:text-7xl font-bold tracking-tight animate-dramatic-fade-in-up bg-gradient-to-r from-accent to-accent-light text-transparent bg-clip-text animate-background-pan bg-[size:200%_auto]" style={{ animationDelay: '200ms' }}>{name}.</h1>;
  const three = <h2 className="text-3xl sm:text-5xl md:text-6xl font-bold text-text-secondary tracking-tight mt-2 animate-dramatic-fade-in-up" style={{ animationDelay: '300ms' }}>I build things for the web and data.</h2>;
